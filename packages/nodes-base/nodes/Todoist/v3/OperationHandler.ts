...
export class MoveHandler implements OperationHandler {
	async handleOperation(ctx: Context, itemIndex: number): Promise<TodoistResponse> {
		...
		// Check which one to set starting from the most fine-grained to more broad
		if (options.parentId) {
			args.parent_id = options.parentId as string;
		} else if (options.sectionId) {
			args.section_id = options.sectionId as string;
		} else if (projectId) {
		...
		ctx.logger.info(JSON.stringify(body.commands[0].args, null, 2));

		const syncResponse = await todoistSyncRequest.call(ctx, body);

		if (syncResponse.sync_status[moveComandUuid] !== 'ok') {
			throw new NodeApiError(ctx.getNode(), syncResponse.sync_status[moveComandUuid] as JsonObject);
		}
